...
from plugins import MangaClient, ManhuaKoClient, MangaCard, MangaChapter, ManhuaPlusClient, TMOClient, MangaDexClient, \
    ...
    KissMangaClient, MangatigreClient, MangaHasuClient, MangaBuddyClient, AsuraScansClient, NineMangaClient, Manhwa18Client, ToonilyClient
import os
...
async def on_private_message(client: Client, message: Message):
    ...
    channel_link = env_vars.get('CHANNEL_LINK')
    if not channel:
...
async def on_start(client: Client, message: Message):
    await message.reply(f"**Hii, {message.from_user.mention}, Welcome to the best manga pdf bot in telegram!!\n"
                        "\n"
                        ...
                        "`One Piece`\n"
                        "\n"
                        "Check /help for more information.**", quote = True)
...
async def on_refresh(client: Client, message: Message):
    ...
    if not (message.reply_to_message and message.reply_to_message.outgoing and
        ...
        return await message.reply("**This command only works when it replies to a manga file that bot sent to you**", quote = True)
    db = DB()
    if document:
    ...
    if not chapter:
        return await message.reply("**This file was already refreshed**", quote = True)
    await db.erase(chapter)
    return await message.reply("**File refreshed successfully!**", quote = True)
...
async def on_subs(client: Client, message: Message):
    ...
    if not lines:
        return await message.reply("**You have no subscriptions yet.**" , quote = True)
...
async def on_cancel_command(client: Client, message: Message):
    ...
    if not sub:
        return await message.reply("**You were not subscribed to that manga.**", quote = True)
    await db.erase(sub)
    return await message.reply("**You will no longer receive updates for that manga.**", quote = True)
...
async def on_options_command(client: Client, message: Message):
    ...
    return await message.reply("**Select the desired output format.**", reply_markup=buttons, quote = True)


@bot.on_message(filters=filters.regex(r'^/'))
async def on_unknown_command(client: Client, message: Message):
    await message.reply("**Unknown command**", quote = True)
